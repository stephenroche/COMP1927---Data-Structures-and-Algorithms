...
{
   List *newChains = malloc(2 * ht->nslots * sizeof(List));
   int i, j, chainLength;
   int *chain;

   for (i = 0; i < 2 * ht->nslots; i++) {
      newChains[i] = newList();
   }

   for (i = 0; i < ht->nslots; i++) {
      chain = valuesFromList(ht->chains[i], &chainLength);

      for (j = 0; j < chainLength; j++) {
         appendList(newChains[hash(chain[j], 2 * ht->nslots)], chain[j]);
      }
   }
