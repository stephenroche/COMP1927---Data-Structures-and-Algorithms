...
static int alreadyConnected(LocationID connectedLocations[MAX_CONNECTIONS], int *numLocations, LocationID location) {
    
    int i;
    for (i = 0; i < *numLocations; i++) {
        if (connectedLocations[i] == location) {
            return 1;
        }
    }
    return 0;
...
LocationID *mapConnectedLocations(Map map, int *numLocations, LocationID from, int player, int round, int road, int rail, int sea) {
   // Feel free to move this to the connectedLocations function in GameView.c
   // It doesn't need to be in Map.c
   // Also could you add some comments to explain what's going on?
   
   LocationID *connectedLocations = malloc(MAX_CONNECTIONS * sizeof(LocationID));
   ...
   // Current location must be in array
   // numLocations keeps track of size of array
   connectedLocations[0] = from;
   *numLocations = 1;
   VList curr, curr2, curr3;
   
   // alreadyConnected() function checks if a connected location has already been added to the array
   
   if (road) {
      curr = map->connections[from];
      
      // Scans through linked list of locations directly connected to from, looks for locations connected by ROAD.
      // Excludes Dracula travelling to the hospital
      
      while (curr != NULL) {
         if (curr->type == ROAD && !alreadyConnected(connectedLocations, numLocations, curr->v) && !(player == PLAYER_DRACULA && curr->v == ST_JOSEPH_AND_ST_MARYS)) {
            connectedLocations[*numLocations] = curr->v;
            (*numLocations)++;
         }
         curr = curr->next;
      }
   ...
   if (rail && player != PLAYER_DRACULA) {
      // Distance traversable by rail dependent on round and player number
      
      int railSpan = (round + player) % 4;
      curr = map->connections[from];
      
      // Conducts a depth-first search of locations connected by RAIL to from, depth of search is dependent on railSpan (0, 1, 2 or 3 rail segments)
      
      while (railSpan > 0 && curr != NULL) {
         if (curr->type == RAIL && !alreadyConnected(connectedLocations, numLocations, curr->v)) {
            connectedLocations[*numLocations] = curr->v;
            (*numLocations)++;
         }
         
         if (curr->type == RAIL && railSpan > 1) {
            curr2 = map->connections[curr->v];
            
            while (curr2 != NULL) {
      
               if (curr2->type == RAIL && !alreadyConnected(connectedLocations, numLocations, curr2->v)) {
                  connectedLocations[*numLocations] = curr2->v;
                  (*numLocations)++;
               }
         
               if (curr2->type == RAIL && railSpan > 2) {
                  curr3 = map->connections[curr2->v];
            
                  while (curr3 != NULL) {
                     
                     if (curr3->type == RAIL && !alreadyConnected(connectedLocations, numLocations, curr3->v)) {
                        connectedLocations[*numLocations] = curr3->v;
                        (*numLocations)++;
                     }
   ...
   if (sea) {
      ...
      // Scans through linked list of locations directly connected to from, looks for locations connected by BOAT.
      
      while (curr != NULL) {
         if (curr->type == BOAT && !alreadyConnected(connectedLocations, numLocations, curr->v)) {
            connectedLocations[*numLocations] = curr->v;
            (*numLocations)++;
         }
         curr = curr->next;
...
int mapDistanceBetween (Map map, LocationID src, LocationID dest, int road, int rail, int sea) {
   if (!(validPlace(src) && validPlace(dest))) {
      return -1;
   } else if (src == dest) {
      return 0;
   }
   ...
   while (!QueueIsEmpty(q)) {
      //showQueue(q);
      curr = QueueLeave(q, &dist);
      
      next = map->connections[curr];
      while (next != NULL) {
         if (seen[next->v] || !((next->type == ROAD && road) || (next->type == RAIL && rail) || (next->type == BOAT && sea))) {
            next = next->next;
            continue;
         }
